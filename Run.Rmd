---
title: "Open Reproducible Data of the OriGami NMR Test for Navigational Map Reading Competencies"
...
On the basis of our location-based game [OriGami](https://origami.ifgi.de) we created a test for measuring navigational map reading (NMR) competencies in children aged six to ten which we called _OriGami NMR Test_. OriGami NMR Test addresses competencies in orientating with a map in a real-world environment. We evaluated the test by conducting a study with 51 primary school students.

The following **evaluation script** gives the reader a detailed insight in:

- The design of OriGami NMR Test and the test score calculation (chapter 2).

- The evaluation of OriGami NMR Test on the basis of the data from the study (chapter 3).

The aim of the script is to enhance the related [publication](https://psyarxiv.com/yps2n/) with details on the test instrument and the test score as well as on the test evaluation. The reader can use the script to reproduce the results of the publication or to develop an own OriGami NMR Test for his/her location or target group of interest (see `README.md` and run `Custom.Rmd` for more details).

# 2 OriGami NMR Test

OriGami NMR Test is designed for measuring navigational map reading (NMR) competencies and based on our location-based game OriGami (implementation details [on Github](https://github.com/origami-team/origami)). For evaluating OriGami NMR Test, we implemented the test for two locations within the OriGami application. In the following, we describe the overall design of the test. 
...
OriGami NMR Test consists of four NMR task types which are ordered in two groups: Location Tasks and Direction Tasks. _Location Tasks_ (fig. 1 left) include tasks that ask to mark the own location on the map (self-location tasks) or to change the location according to the map (navigation-to-a-flag tasks). _Direction Tasks_ (fig. 1 right) cover tasks that require to mark the current view direction on the map (mark-view-direction tasks) or to change it according to the map (adopt-view-direction tasks). Between these tasks so-called _Transition Tasks_ allow a coherent flow of the test (fig. 1 grey boxes).
...
For further information on the test execution, see the related [publication](https://psyarxiv.com/yps2n/).
...
Details on the description and interpretation of the results are presented in the corresponding [publication](https://psyarxiv.com/yps2n/). In this script the focus is on data details, the score calculations, and the test evaluation calculations.
