...
In the following, the raw data, task performance data, as well as the resulting NMR competency test score for the location _`r location`_ with `r participants` participants is presented.
...
We fixed the _thresholds_ (chapter 2.2.4) to the following values resulting to the NMR competency test scoring presented below. The thresholds define the "line" between a correct and incorrect as well as a confident and inconfident solution. The scoring data can be found in the files `scoring_loc.Rda` or `scoring_loc.json` (for Location Tasks) and `scoring_dir.Rda` or `scoring_dir.json` (for Direction Tasks) of the `results` folder.
