...
In the following, the distribution of the OriGami NMR Test Score for `r location` is presented.
...
The test validity is determined by correlating the OriGami NMR Test scoring with the expert judgment, which is visualized in the following.
...
```
`r knitr::knit(text = unlist(src))`
_Fig. `r 6 - figmin` - `r location`: Validity Coefficients._
